...
public:
    ...
    int addKeySlotByKeyFileDeviceOffset(int keyslot,const char *keyfile,size_t keyfile_size,uint64_t keyfile_offset,const char *new_keyfile,size_t new_keyfile_size,uint64_t new_keyfile_offset);
    ...
    int decrypt(unsigned char *dst,  size_t dstLength, const char* cipher, const char* cipherMode,StorageKey *vk, unsigned int sector);
...
private:
    int decryptBlockwise(int in, int out,LuksStorage* luksStorage,  size_t dstLength,  unsigned int inSector, unsigned int outSector);
    int encryptBlockwise(int in, int out,LuksStorage* luksStorage,  size_t dstLength,  unsigned int inSector, unsigned int outSector);
    char *type;
    char *path;
