...
int LuksPartitionHeader::generateHeader(LuksPartitionHeader *header, StorageKey *storageKey,uint32_t digestIter,
                                        const char *cipherName, const char *cipherMode, const char *hashSpec, const char *uuid, unsigned int stripes, unsigned int alignPayload, unsigned int alignOffset,
    ...
    if (alignPayload && detached_metadata_device && alignPayload < hdr_sectors) {
        Logger::error("Data offset for detached LUKS header must be ", "either 0 or higher than header size (%d sectors).",hdr_sectors);
        return -EINVAL;
    }
